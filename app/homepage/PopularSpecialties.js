...
const specialties = [
  ...
  { name: "Ortodoncia Infantil", imgSrc: "/assets/PopularSpecialties1.svg" },
  {
    ...
    imgSrc: "/assets/PopularSpecialties1.svg",
  },
...
const PopularSpecialties = () => {
  ...
  const handleSeeLess = () => {
    setVisibleSpecialties(specialties.slice(0, 12));
    setShowAll(false);
  };
  return (
    <>
      <div className="text-center max-w-[1440px] mx-auto w-full py-[60px] px-2 sm:px-6">
        <h2 className="text-[#333333] text-[28px] leading-[28px] md:leading-[54px] font-bold">
          ESPECIALIDADES POPULARES
        </h2>
        <p className="text-[#263238] text-[14px] leading-[19px] font-normal mt-[8px]">
          Book and manage your dental appointment in one go
        </p>
        <div className="max-w-[1123px] mx-auto my-[40px] flex flex-wrap justify-center items-start gap-[55px]">
          {visibleSpecialties.map((specialty, index) => (
            <div
              key={index}
              className="flex flex-col gap-2.5 max-w-[160px] items-center"
            >
        ...
        {showAll ? (
          <button
            ...
            className="w-[157px] h-[41px] rounded-[10px] border border-[#263238] bg-white text-[#263238] text-[14px] leading-[14.45px] font-bold"
          >
            See Less
          </button>
        ) : (
        ...
        <div className="max-w-[959px] mx-auto w-full pt-10 grid grid-cols-2 md:grid-cols-3 gap-10">
          {insuranceItems.map((insurance, index) => (
            <div key={index} className="flex items-center gap-3">
              ...
              />
              <h2 className="text-[#263238] text-[13px] md:text-[18px] leading-[19px] font-medium">
                {insurance.name}
              </h2>
            </div>
          ))}
          <button className="md:flex hidden text-left text-[#2C64D8] text-[16px] leading-[19px] font-normal items-center gap-2">
            25 More <IoArrowForward className="" />
          </button>
        </div>
        <div className="mt-[30px] flex items-center justify-center">
          <button className="flex md:hidden w-[157px] justify-center h-[41px] rounded-[10px] text-[#263238] text-[14px] leading-[14px] font-normal items-center gap-2 border border-[#263238]">
            See All
          </button>
        </div>
      </div>
